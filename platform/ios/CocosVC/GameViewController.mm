...
//        [g_vc showCoupon];
    ...
    CocosAppDelegate *app_delegate = (CocosAppDelegate*) app;
    app_delegate->setupMainScene();
        ...
        // Enable or disable multiple touches
        [self.eaglView setMultipleTouchEnabled:YES];
        
        [self.view insertSubview:self.eaglView atIndex:0];
        
        // menu button 退出
        [self.menuButton addTarget:self action:@selector(closeGameScene) forControlEvents:UIControlEventTouchUpInside];
        ...
        if(self.demoNum == 2){
            CocosAppDelegate *app_delegate = (CocosAppDelegate*) app;
            app_delegate->setupDemoTwoScene();
        }
        if(self.demoNum == 3){
            CocosAppDelegate *app_delegate = (CocosAppDelegate*) app;
            app_delegate->setupDemoThreeScene();
        }
    }
